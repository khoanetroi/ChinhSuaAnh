# -*- coding: utf-8 -*-
"""
FaceBeautifyImageView.py - View for face beautification from image
Part of MVC architecture - handles UI for face beautification features
"""
...
class FaceBeautifyImageView:
    """View for face detection and beautification from static images"""

    def __init__(self, parent_controller, original_image):
        """
        Initialize Face Beautify Image View

        Args:
            parent_controller: Reference to MainController
            original_image: Original image from main application
        """
    ...
    def reset_to_default(self):
        """Reset image to original and values to defaults (keep face detection)"""
        if not self.check_image():
            return
    ...
    def apply_changes(self):
        """Apply changes back to main application"""
        if not self.check_image():
            return
        ...
        if result:
            self.controller.image_service.load_image(self.current_image.copy(), "face_beautified")
            self.controller._update_ui()
            self.controller.view.update_status("✓ Đã áp dụng làm đẹp khuôn mặt")
